...
async function collectDocuments(options: Required<DocfindPluginOptions>) {
  ...
  for (const entry of entries) {
    ...
    const title = typeof parsed.data?.title === 'string' ? parsed.data.title : inferTitle(parsed.content);
    const category = typeof parsed.data?.category === 'string' ? parsed.data.category : undefined;
...
export function docfindPlugin(options: DocfindPluginOptions = {}): Plugin {
  ...
  const resolvedOutDir = path.resolve(cwd, options.outDir ?? path.join(resolvedDocsDir, '.vitepress/dist'));
  const resolvedIndexDir = path.resolve(cwd, options.indexDir ?? path.join(resolvedOutDir, 'docfind'));
